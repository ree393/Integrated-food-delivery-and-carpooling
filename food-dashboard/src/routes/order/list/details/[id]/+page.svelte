<script lang="ts">
    ...
    import type { IOrder } from "$lib/events/models/IOrder";
    import { FirebaseOrderRepo } from "$lib/events/repo/FirebaseEventRepo";
    import MapCon from "$lib/events/ui-components/MapCon.svelte";
    import OrderMap from "$lib/events/ui-components/OrderMap.svelte";
    import { mapOrderItemsForMap } from "$lib/events/utils/events-utils";
    ...
    onMount(async () => {
        let data = await eventRepo.getOrderById($page.params.id);
        if (data) {
            order = data;
        }
    });
...
<div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
    ...
    {#if order}
        ...
        >
            ...
            >
                <div
                    class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-800 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full"
                >
                    <p
                        class="text-lg md:text-xl  font-semibold leading-6 xl:leading-5 text-white"
                    >
                        Customer’s Details
                    </p>

                    <div class="my-7 text-white space-y-3 w-full">
                        <h3>
                        ...
                        >
                            <div class="pb-4 md:pb-8 w-full md:w-40">
                                <img
                                    ...
                                    src="https://toppng.com/uploads/preview/clipart-free-seaweed-clipart-draw-food-placeholder-11562968708qhzooxrjly.png"
                                    alt="dress"
                                />
                                <img
                    ...
                    {/each}
                    {#if order.type === "Ride"}
                        <h3 class="text-white my-5">Destination Location :</h3>
                        {#if order?.location && browser}
                            <MapCon lat={order.location.lat} lng={order.location.long}></MapCon>
                        {/if}
                    {/if}
                </div>
                <div
            ...
            >
                ...
                >
                    <div class="flex flex-col justify-start items-start flex-shrink-0">
                        ...
                        >
                            <img class="h-[4rem] w-[4rem] rounded-full" src="https://png.pngtree.com/element_our/png/20181206/users-vector-icon-png_260862.jpg" alt="avatar" />
                            <div class="flex justify-start items-start flex-col space-y-2">
                                <p
